...
sss, rtcbs, rtcrs = double_init(branches1.ss_val_on, upper_ranges, branches1.ss_val_off, "rtcr", "rtcb", ps)
binary=get_binary(branches1.ss_val_off[7], sss)

binary_matrix = reshape(binary, (l1,l1))
...
xs,ys = get_xy_percentage(set_shared_range_0ton(n1,l1))
new_arr = reshape(tuple.(xs,ys), (l1,l1))
...
x1 = 100 .- x
y1 = 100 .- y
plot(scatter(x=x1,y=y1), Layout(xaxis_range=(0,100), yaxis_range=(0,100)))
p = plot_binary(sss, range(-100,0,length=l1), branches1.ss_val_off[7], "rtcb", "rtcr", "upper to lower - rh, kdam = 1")
...
for kdam_val in ProgressBar(kdam_range)
    ...
    sss, rtcbs, rtcrs = double_init(branches1.ss_val_on, upper_ranges, branches1.ss_val_off, "rtcb", "rtcr", ps)
    push!(sss_res, sss)
