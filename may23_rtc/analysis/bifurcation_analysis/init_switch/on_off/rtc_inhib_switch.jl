...
rtcb10 = scatter(x=df0.kdam[1:kdam10], y=df0.rtcb[1:kdam10], name="RtcB", line=attr(width=3, color=:green), showlegend=false, legendgroup="1")#, fill="tozeroy")
rtcb20 = scatter(x=df0.kdam[kdam10:kdam20], y=df0.rtcb[kdam10:kdam20], name="", line=attr(width=3,dash="dash", color=:black),showlegend=false, legendgroup="1")
rtcb30 = scatter(x=df0.kdam[kdam20:end], y=df0.rtcb[kdam20:end], name="", line=attr(width=3, color=:red),showlegend=false, legendgroup="1")
bf_rtcb0 = scatter(x=bf0.kdam, y=bf0.rtcb, mode="markers", name="Bifurcation point", line=attr(color=:black),showlegend=false, legendgroup="1")
plot([rtcb10, rtcb20, rtcb30, bf_rtcb0])
...
krep = 137., ktag = 9780., atp = 3578.9473684210525, km_a = 20., km_b = 16., g_max = 2.0923, 
kdeg = 0.001, kin = 0.022222222, ω_ab = 0.05623413251903491, ω_r = 0.010000000000000002, 
kdam =  0.01, lam = 0.014, k_inhib=k_inhib, inhib=inhib)
...
br = get_br(rtc_inhib_mod, params_for_ssval_setup_inhib, initial, 3.)
bf = bf_point_df(br)
df = create_br_df(br)
...
for kdam_val in ProgressBar(kdam_range)
    ...
    psm.kdam = kdam_val
    branches1 = setup_ssvals_from_bfkit(rtc_inhib_mod, kdam_val, params_for_ssval_setup_inhib)
    # @show psm
    
    n = 600; l = 500;
    upper_ranges = get_all_ranges(set_ss_range_zerotossval, branches1, "ss_val_on", n, l)
    # @show upper_ranges[4]
    all, init_vals = get_rh_init_switch_all_ranges(rtc_inhib_model, upper_ranges, branches1.ss_val_on,:rh,l,psm)
