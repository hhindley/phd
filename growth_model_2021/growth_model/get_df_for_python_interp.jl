...
sf_range = collect(10:1000:100000)
...
param_range = collect(1000:500:100000)
max_vals = max_vals_param_sweep(:ns, param_range, ssinit)
max_vals = max_vals_param_sweep(:Kgamma, Kgamma_range, ssinit)

for i in Kgamma_range
    if 0.01 < max_vals[i][1]["λ"][1] && max_vals[i][1]["ATP"][1] < 20000
        println("ns = ", i, " and ", max_vals[i][1])
    end
end
...
first_a = a[1:argmax(a)+1]
last_a = a[argmax(a)+1:end]
first_lam = lam[1:argmax(a)+1]
last_lam = lam[argmax(a)+1:end]
...
dlam = new_df."gr"
first_dlam = dlam[1:argmax(dlam)-1]
last_dlam = dlam[argmax(dlam):end]
lam1 = scatter(x=new_df.t[1:argmax(dlam)-1], y=first_dlam)
...
atp2 = scatter(x=new_df.t[argmax(dlam):end], y=atp_rtc_last)
plot([atp1,atp2], Layout(xaxis_range=(0,1000)))

# rejoin sections for full atp array 
new_atp = [atp_rtc_first; atp_rtc_last]
plot(scatter(x=new_df.t, y=new_atp), Layout(xaxis_range=(0,1300)))
