...
p_rtc1 = plot([scatter(x=df_rtc.time, y=col, name="$(names(df_rtc)[i])", legendgroup="$i", marker_color=colours[i]) for (col, i) in zip(eachcol(df_rtc[:,2:end]), range(2,length(names(df_rtc))))], Layout(title="kdam = $(params_rtc[kdam])"))

params_kdam = deepcopy(params_rtc_molec)
params_kdam[kdam] = 0.1
solu_rtc_dam = sol(rtc_model, ssvals_rtc_molec, (0,200*log(2)/lam_val), params_kdam)
...
tlr_el = par[pidx(:g_max)]*atp_val_molec/((par[pidx(:θtlr)])+atp_val_molec)

tlr_a = @. (1/par[pidx(:na)])*par[pidx(:kc)]*df_rtc.rh*df_rtc.rm_a*tlr_el
tlr_b = @. (1/par[pidx(:nb)])*par[pidx(:kc)]*df_rtc.rh*df_rtc.rm_b*tlr_el
tlr_r = @. (1/par[pidx(:nr)])*par[pidx(:kc)]*df_rtc.rh*df_rtc.rm_r*tlr_el

Vrep = @. df_rtc.rtcb*df_rtc.rt*par[pidx(:krep)]/(df_rtc.rt+par[pidx(:km_b)]) # uM min-1 
Vtag = @. df_rtc.rtca*df_rtc.rd*par[pidx(:ktag)]/(df_rtc.rd+par[pidx(:km_a)]) # uM min-1 

Vdam = @. par[pidx(:kdam)]*df_rtc.rh # uM min-1

Vinflux = par[pidx(:kin)] * par[pidx(:g_max)]*atp_val_molec/(par[pidx(:θtlr)]+atp_val_molec) # uM min-1 
