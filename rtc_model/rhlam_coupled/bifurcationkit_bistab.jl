...
br = get_br(test, init_rtc, params_rtc1, 1.5)
bf = bf_point_df(br)
df = create_br_df(br)
kdam1 = findall(x->x==bf.kdam[1],df.kdam)[1]
kdam2 = findall(x->x==bf.kdam[2],df.kdam)[1]
...
prob_bf = ODEProblem(test, [0,0,0,0,0,0,10,0,0], tspan, params_rtc1; jac=true)
odefun = prob_bf.f
F = (u,p) -> odefun(u,p,0)
...
opts_br = ContinuationPar(p_min = 0., p_max = 1., ds = 1e-4, #a=0.1,
dsmax = 1e-3, dsmin = 1e-6, # 0.15
