...
@parameters L c kr Vmax_init Km_init ω_ab ω_r θtscr g_max θtlr km_a km_b d krep kdam ktag kdeg kin_c atp na nb nr lam_c kc k_diss offset
species_rtc1 = @syms rm_a(t) rtca(t) rm_b(t) rtcb(t) rm_r(t) rtcr(t) rh(t) rd(t) rt(t) 
species_rtc = [Symbol(i) for i in species_rtc1]
  
D = Differential(t)

@mtkmodel LAMKIN_COUPLED begin
    @parameters begin
        L 
        c 
        kr
    ...
    @equations begin
        ...
        rhs_rtca ~ tlr(rm_a, na, rh_on, tlr_el) - dil(rtca,lam)     
        rhs_rm_b ~ tscr_ab - dil(rm_b,lam) - deg(rm_b)
        rhs_rtcb ~ tlr(rm_b, nb, rh_on, tlr_el) - dil(rtcb,lam)
        rhs_rm_r ~ tscr_r - dil(rm_r,lam) - deg(rm_r)
        rhs_rtcr ~ tlr(rm_r, nr, rh_on, tlr_el) - dil(rtcr,lam)
...
@mtkbuild lamkin_coupled = LAMKIN_COUPLED()
...
atp=>atp_val, km_a=>km_a_val, km_b=>km_b_val, g_max=>g_max_val, kdeg=>kdeg_val, kin_c=>kin_c_val, ω_ab=>ω_ab_val, ω_r=>ω_r_val, kdam=>kdam_val, lam_c=>lam_c_val, kc=>kc_val, k_diss=>k_diss_val, offset=>offset_val)

init_rtc = [lamkin_coupled.rm_a=>0.0,lamkin_coupled.rtca=>0.0,lamkin_coupled.rm_b=>0.0,lamkin_coupled.rtcb=>0.0,lamkin_coupled.rm_r=>0.0,lamkin_coupled.rtcr=>0.0,lamkin_coupled.rh=>11.29,lamkin_coupled.rd=>0.0,lamkin_coupled.rt=>0.0]

ssvals_rtc = steady_states(lamkin_coupled, init_rtc, params_rtc1)

kdam_range = range(0,100, length=100) 

df_ssvals = var_param(lamkin_coupled, kdam, params_rtc1, kdam_range, ssvals_rtc)

jac_sym=calculate_jacobian(lamkin_coupled)

model = lamkin_coupled

using GLMakie
include(joinpath(homedir(), "phd/rtc_model/functions/bf_funcs/bf_funcs.jl"))
lines(kdam_range, df_ssvals.rtca)

res = numerical_bistability_analysis(model, params_rtc1, init_rtc, species_rtc, kdam_range, kdam)
res1 = numerical_bistability_analysis(model, params_rtc1, ssvals_rtc, species_rtc, reverse(kdam_range), kdam)

lines(kdam_range, res.rtca)
lines!(reverse(kdam_range), res1.rtca)
