...
function compare_data_and_sol(model, tspan, t, ω_ab, kdam, curve, data)
    obj = []
    solu = sol_with_t(model, init, tspan, (@SVector [L, c, kr, Vmax_init, Km_init, ω_ab, ω_r, θtscr, g_max, θtlr, km, k_b, gr_c, d, krep, kdam, ktag, kdeg, kin, atp, na, nb, nr]), t)
    if curve == "mrnas"
        rm_a = get_curve(solu, :rm_a)
        rm_b = get_curve(solu, :rm_b)
        append!(obj, [abs2(i-j) for (i,j) in zip(rm_a, data)])
        append!(obj, [abs2(i-j) for (i,j) in zip(rm_b, data)])
    elseif curve == "rh"
        rh = get_curve(solu, :rh)
        append!(obj, [abs2(i-j) for (i,j) in zip(rh, data)])
    end
    return obj
end
