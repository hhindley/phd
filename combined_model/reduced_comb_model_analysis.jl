...
include("$PATH/combined_model/reduced_comb_model.jl")
...
# stability analysis 
abx_range = range(0,100,length=50)
...
wr_range = 10 .^ range(log10(1e-5),log10(1e-1),length=5)
n_params = deepcopy(params_red_comb_new)
n_params1 = deepcopy(params_red_comb_new)
for i in wab_range
    n_params[w_BA] = i
    for j in wr_range
        n_params[w_R] = j
        ssvals_red_comb_new = steady_states(reduced_combined_model, init_red_comb, n_params)
        ssvals = sweep_param(reduced_combined_model, ssvals_red_comb_new, n_params, abx_range, kdam, species_red_comb)
        display(plot(scatter(x=abx_range, y=ssvals[!,:rh]), Layout(title="ω_ab = $i, ω_r = $j")))
    end
end


sweep_abx = sweep_param_with_plot(reduced_combined_model, ssvals_red_comb_new, params_red_comb_new, abx_range, kdam, species_red_comb, "")
