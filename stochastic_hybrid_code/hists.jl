...
dfs = [DataFrame(CSV.File(joinpath("$PATH/stochastic_hybrid/thresh_testing3", file), header=["event", "time", "rm_a", "rtca", "rm_b", "rtcb", "rm_r", "rtcr", "rh", "rd", "rt", "volume"])) for file in readdir("$PATH/stochastic_hybrid/thresh_testing3")]
...
for i in range(1,length(dfs))
    ...
    push!(traces_rma,plot(histogram(x=df_grouped.rm_a./df_grouped.volume, name="$(round.(df_time.threshold[i], digits=3))", nbinsx=50)))
    push!(traces_rh,plot(histogram(x=df_grouped.rh./df_grouped.volume, name="$(round.(df_time.threshold[i], digits=3))", nbinsx=50)))
end
