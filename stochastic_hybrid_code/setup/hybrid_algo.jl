...
function hybrid_algo(X0, options, prop, S; out=stdout)
    ...
    while ~division && (tf > s0)
        ...
        solu = solve(prob, Rodas4(), callback=CallbackSet(division_cb, stochreact_cb, sampling_cb), tstops = div_times, save_everystep=false)#, abstol=1e-12, reltol=1e-9, dtmax=0.05) # solve for deterministic reactions and stop when one of the callbacks is reached
