...
for i in ProgressBar(threshold_vals)
    time_taken = @elapsed run_stoch(X0, i, 0.05, "/home/hollie_hindley/Documents/stochastic_hybrid/thresh_1906/thresh_$i.dat")
    push!(times, time_taken)
end

df = DataFrame(threshold=threshold_vals, time=times)

CSV.write("/home/hollie_hindley/Documents/stochastic_hybrid/thresh_times_1906.csv", df)

println("total time = $(sum(times)/60/60) hours")

arrow_conv(joinpath(homedir(), "Documents/stochastic_hybrid/thresh_1906"), joinpath(homedir(), "Documents/stochastic_hybrid/thresh_1906_final_files"))
