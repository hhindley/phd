...
df_reacts0 = load_files("/Users/s2257179/stoch_files/thresh_test_arrow_files_14_06/reacts")
df_reacts0 = [DataFrame(df_reacts0[i]) for i in eachindex(df_reacts0)]
...
for i in eachindex(df_reacts0)
    for num in 1:13
        if num ∉ df_reacts0[i].event
            push!(df_reacts0[i], (event=num, count=0, reaction=react_names[num]))
        end
    end
end

df_reacts = load_files("/Users/s2257179/stoch_files/thresh_1906_final_files/reacts")
