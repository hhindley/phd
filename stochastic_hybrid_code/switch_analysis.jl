...
function switch_times(res; threshold=nothing)
    ...
    nan_row = DataFrame(on=NaN, time=NaN, rtca=NaN, rtcb=NaN)
    df = vcat(nan_row, df)
    return df, start_times, stop_times
end
threshold=2
df, start_times, stop_times = switch_times(dict_results[folder][6], threshold=threshold)
df1, start_times1, stop_times1 = switch_times(dict_results[folder][6])

function calc_switch_rate(start_times, stop_times)
...
switch_rate_on, switch_rate_off = calc_switch_rate(start_times, stop_times)
switch_rate_on1, switch_rate_off1 = calc_switch_rate(start_times1, stop_times1)


# plot switch times
f = Figure()
ax = Axis(f[1,1], title="Threshold for on state = $threshold, time on = $(round(time_on, digits=2)), switch rate = $(round(switch_rate_on, digits=6))", xlabel="time", ylabel="molecule number")
...
f = Figure()
ax = Axis(f[1,1], xlabel="kdam", ylabel="switch rate", title="threshold method", yscale=log10)
...
f = Figure()
ax = Axis(f[1,1], xlabel="kdam", ylabel="fraction of time in state", title="threshold method")
lines!(ax, dict_kdamvals[folder][:kdam], frac_on, label="on state")
lines!(ax, dict_kdamvals[folder][:kdam], frac_off, label="off state")
axislegend()
