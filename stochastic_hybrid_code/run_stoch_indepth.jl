...
function run_stoch(X0, thresh, kdam)
    ...
    # df = DataFrame(CSV.File("$PATH/stochastic_hybrid/test1.dat", header=["time", "rm_a", "rh", "TotProp"]))
...
react_names = [:tscr_ab, :tscr_r, :tlr_a, :tlr_b, :tlr_r, :Vinflux, :Vdam, :Vtag, :Vrep, :dil_rh, :dil_rd, :dil_rt, :dil_rma, :dil_rmb, :dil_rmr, :dil_rtca, :dil_rtcb, :dil_rtcr, :deg_rd, :deg_rma, :deg_rmb, :deg_rmr]
df_react = DataFrame([name => Float64[] for name in react_names])
for vec in df.event[2:end]
    push!(df_react, vec)
end

p_props = plot([scattergl(x=df.time, y=df_react[:,col], mode="markers", name="$(names(df_react)[i])", legendgroup="$i", showlegend=true) for (col, i) in zip(names(eachcol(df_react)), range(1,length(names(df_react))))], Layout(xaxis_title="reaction propensities", yaxis_title="time", yaxis_tickformat=".2e", title="Reaction propensities - high stoch"))#, yaxis_type="log"));#, title="kdam = $(params_rtc[kdam])"))
add_trace!(p_props, scattergl(x=[df.time[2],df.time[end]], y=[0,0], mode="lines", marker_color="black", name="threshold", marker_width=4))
p_props


high_thresh = 5
df3 = @time run_stoch(X0, low_thresh, 0.1)
...
p_species1 = plot([scattergl(x=df4.time, y=df4[:,col], name="$(names(df4)[i])", marker_color=colours[i], legendgroup="$i", showlegend=true) for (col, i) in zip(names(eachcol(df4[:,2:end-1])), range(2,length(names(df4))-1))], Layout(yaxis_tickformat=".2e", title="low threshold"))#, title="kdam = $(params_rtc[kdam])"))
